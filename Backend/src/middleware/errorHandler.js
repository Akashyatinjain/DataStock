  ...
  if (err instanceof multer.MulterError) {
    switch (err.code) {
      case "LIMIT_FILE_SIZE":
        status = 413;
        code = "FILE_TOO_LARGE";
        // Try to read the configured limit from multer
        const maxSize = err.storageErrors?.[0]?.storageErrors || 10 * 1024 * 1024;
        message = `File is too large. Maximum allowed size is ${formatBytes(10 * 1024 * 1024)}.`;
